...
class game ():  
	...
	def SmartMoveScreen (self):
		...
		possibleScreenX = pacman.x - self.screenTileSize[1] / 2 * TILE_WIDTH
		possibleScreenY = pacman.y - self.screenTileSize[0] / 2 * TILE_HEIGHT
	...
	def SetNextLevel (self):
		...
		pacman.velX = 0
		pacman.velY = 0
		pacman.anim_pacmanCurrent = pacman.anim_pacmanS
		pacman2.velX = 0
		pacman2.velY = 0
		pacman2.anim_pacmanCurrent = pacman2.anim_pacmanS
...
class ghost ():
	...
	def Draw (self):
		...
		if pacman2.x > self.x and pacman2.y > self.y:
			#pacman is to lower-right
			pupilSet = (8,9)
		elif pacman2.x < self.x and pacman2.y > self.y:
			#pacman is to lower-left
			pupilSet = (5,9)
		elif pacman2.x > self.x and pacman2.y < self.y:
			#pacman is to upper-right
	...
	def Move (self):
		...
		if (self.x % TILE_WIDTH) == 0 and (self.y % TILE_HEIGHT) == 0:
			...
			if len(self.currentPath) > 0:
				...
				# chase pac-man
				self.currentPath = path.FindPath( (self.nearestRow, self.nearestCol), (pacman.nearestRow, pacman.nearestCol) )
				self.FollowNextPathWay()
		...
		if not self.currentPath == False:
			...
			if len(self.currentPath) > 0:
				...
				if not self.state == 3:
					# chase pac-man
					self.currentPath = path.FindPath( (self.nearestRow, self.nearestCol), (pacman.nearestRow, pacman.nearestCol) )
					self.currentPath = path.FindPath( (self.nearestRow, self.nearestCol), (pacman2.nearestRow, pacman2.nearestCol) )
					self.FollowNextPathWay()
...
class level ():
	...
	def CheckIfHitWall (self, (possiblepacmanX, possiblepacmanY), (row, col)):
		...
		for iRow in range(row - 1, row + 2, 1):
			for iCol in range(col - 1, col + 2, 1):
				...
				if  (possiblepacmanX - (iCol * TILE_WIDTH) < TILE_WIDTH) and (possiblepacmanX - (iCol * TILE_WIDTH) > -TILE_WIDTH) and (possiblepacmanY - (iRow * TILE_HEIGHT) < TILE_HEIGHT) and (possiblepacmanY - (iRow * TILE_HEIGHT) > -TILE_HEIGHT):
					
					if self.IsWall((iRow, iCol)):
						numCollisions += 1
						
	def CheckIfHitWall (self, (possiblepacman2X, possiblepacman2Y), (row, col)):
                numCollisions = 0
		...
		for iRow in range(row - 1, row + 2, 1):
			for iCol in range(col - 1, col + 2, 1):
				...
				if  (possiblepacman2X - (iCol * TILE_WIDTH) < TILE_WIDTH) and (possiblepacman2X - (iCol * TILE_WIDTH) > -TILE_WIDTH) and (possiblepacman2Y - (iRow * TILE_HEIGHT) < TILE_HEIGHT) and (possiblepacman2Y - (iRow * TILE_HEIGHT) > -TILE_HEIGHT):
	...
	def CheckIfHit (self, (pacmanX, pacmanY), (x, y), cushion):
	
		if (pacmanX - x < cushion) and (pacmanX - x > -cushion) and (pacmanY - y < cushion) and (pacmanY - y > -cushion):
			return True
		else:
			return False

	def CheckIfHit (self, (pacman2X, pacman2Y), (x, y), cushion):
	
		if (pacman2X - x < cushion) and (pacman2X - x > -cushion) and (pacman2Y - y < cushion) and (pacman2Y - y > -cushion):
			return True
		else:
			return False	


	def CheckIfHitSomething (self, (pacmanX, pacmanY), (row, col)):
	
		for iRow in range(row - 1, row + 2, 1):
			for iCol in range(col - 1, col + 2, 1):
			
				if  (pacmanX - (iCol * TILE_WIDTH) < TILE_WIDTH) and (pacmanX - (iCol * TILE_WIDTH) > -TILE_WIDTH) and (pacmanY - (iRow * TILE_HEIGHT) < TILE_HEIGHT) and (pacmanY - (iRow * TILE_HEIGHT) > -TILE_HEIGHT):
					# check the offending tile ID
					...
					if result == tileID[ 'pellet' ]:
						...
						snd_pellet[pacman.pelletSndNum].play()
						pacman.pelletSndNum = 1 - pacman.pelletSndNum
						snd_pellet[pacman2.pelletSndNum].play()
						pacman2.pelletSndNum = 1 - pacman2.pelletSndNum
						...
						for i in range(0, thisLevel.lvlWidth, 1):
							if not i == iCol:
								if thisLevel.GetMapTile((iRow, i)) == tileID[ 'door-h' ]:
									...
									pacman2.x = i * TILE_WIDTH
									
									if pacman2.velX > 0:
										pacman2.x += TILE_WIDTH
									else:
										pacman2.x -= TILE_WIDTH
						...
						for i in range(0, thisLevel.lvlHeight, 1):
							if not i == iRow:
								if thisLevel.GetMapTile((i, iCol)) == tileID[ 'door-v' ]:
									pacman.y = i * TILE_HEIGHT
									
									if pacman.velY > 0:
										pacman.y += TILE_HEIGHT
									else:
										pacman.y -= TILE_HEIGHT

									pacman2.y = i * TILE_HEIGHT

									if pacman2.velY > 0:
										pacman2.y += TILE_HEIGHT
									else:
										pacman2.y -= TILE_HEIGHT
				...
				if isReadingLevelData == True:
					...
					for k in range(0, self.lvlWidth, 1):
						...
						if thisID == 4: 
							...
							pacman.homeX = k * TILE_WIDTH
							pacman.homeY = rowNum * TILE_HEIGHT
							pacman2.homeX = k * TILE_WIDTH
							pacman2.homeY = rowNum * TILE_HEIGHT
	...
	def Restart (self):
		...
		pacman.x = pacman.homeX
		pacman.y = pacman.homeY
		pacman.velX = 0
		pacman.velY = 0
		...
		pacman2.x = pacman2.homeX
		pacman2.y = pacman2.homeY
		pacman2.velX = 0
		pacman2.velY = 0
...
def CheckInputs(): 
	...
	if thisGame.mode == 1:
		if pygame.key.get_pressed()[ pygame.K_RIGHT ] or (js!=None and js.get_axis(JS_XAXIS)>0.5):
			if not (pacman.velX == pacman.speed and pacman.velY == 0) and not thisLevel.CheckIfHitWall((pacman.x + pacman.speed, pacman.y), (pacman.nearestRow, pacman.nearestCol)): 
			...
			if not (pacman.velX == 0 and pacman.velY == pacman.speed) and not thisLevel.CheckIfHitWall((pacman.x, pacman.y + pacman.speed), (pacman.nearestRow, pacman.nearestCol)): 
				pacman.velX = 0
		...
		if pygame.key.get_pressed()[ pygame.K_d ] or (js!=None and js.get_axis(JS_XAXIS)>0.5):
			if not (pacman2.velX == pacman2.speed and pacman2.velY == 0) and not thisLevel.CheckIfHitWall((pacman2.x + pacman2.speed, pacman2.y), (pacman2.nearestRow, pacman2.nearestCol)): 
				pacman2.velX = pacman2.speed
				pacman2.velY = 0
			...
			if not (pacman2.velX == 0 and pacman2.velY == pacman2.speed) and not thisLevel.CheckIfHitWall((pacman2.x, pacman2.y + pacman2.speed), (pacman2.nearestRow, pacman2.nearestCol)): 
				pacman2.velX = 0
				pacman2.velY = pacman2.speed
...
while True: 
	...
	if thisGame.mode == 5:
		thisGame.DrawNumber (thisGame.ghostValue / 2, (pacman.x - thisGame.screenPixelPos[0] - 4, pacman.y - thisGame.screenPixelPos[1] + 6))
		thisGame.DrawNumber (thisGame.ghostValue / 2, (pacman2.x - thisGame.screenPixelPos[0] - 4, pacman2.y - thisGame.screenPixelPos[1] + 6))
